...
namespace RedTelephone.Controllers
{
    ...
    {
        ObjectSet<Priority> table = (new ModelsDataContext()).Priorities;
        String className = "PrioritiesController";
            ...
            return authenticatedAction(new String[] { "UR" }, () => formAction(
                    ...
                    ()=> sideEffectingAction(() => {
                        ...
                        foreach (KeyValuePair<String, Dictionary<String, String>> itemPair in formVars) {
                            ...
                            Priority possibleItem = table.FirstOrDefault(p => p.code == code);

                            //VALIDATION HAPPENS HERE
                            validationLogPrefix = className + ".Index";
                            ValidateStrLen(itemPair.Value["description"], 32, "Priority descriptions");
                            //AND THEN ENDS.
